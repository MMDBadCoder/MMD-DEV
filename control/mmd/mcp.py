...
# Statuses the agent may set. CLOSED is deliberately absent.
AGENT_STATUSES = {"in_progress", "answered"}
...
def reply_to_ticket(db: Session, ticket_id: int, body: str,
    ...
    if status is not None and status not in AGENT_STATUSES:
        ...
        raise McpError(
            ...
            f"is a human decision, so the agent cannot do it.")
...
TOOLS = [
    ...
    },
    {
        ...
        "description": (
            "Post a reply to one ticket, as the support agent. Optionally set "
            "the status to 'in_progress' (you are working on it) or "
            "'answered' (you believe it is resolved). You cannot close a "
            "ticket - a human decides that. Reply in the customer's own "
            "language; these customers write Persian."),
        "inputSchema": {
            "type": "object",
            "properties": {
...
def handle(db: Session, request: dict) -> dict | None:
