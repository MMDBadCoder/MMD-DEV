...
def _hermes_install(db, ws: Workspace) -> None:
    ...
    if not ws.hermes_enabled:
        ...
        db.commit()
        return
    # Deliberately NOT skipped when "already installed". The customer can
    # change Hermes' own config at any time - switching the webhook platform
    # on in its dashboard, for instance - and that state lives inside the
    # machine, so this side cannot know whether a reconcile is needed without
    # running one. Skipping here once meant the gateway was never installed
    # for a customer who had enabled the webhook, with nothing to show why.
    #
    # Running every pass is safe because the provisioner is idempotent: it
    # compares the unit file and only restarts the dashboard when something
