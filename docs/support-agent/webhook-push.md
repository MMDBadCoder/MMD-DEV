...
It is genuinely optional. A nudge that never arrives loses nothing — the
ticket is still in the queue and the next timer tick finds it — so nothing
here affects whether tickets get answered, only how quickly.

It also needs the agent to run *inside* a machine with a working MCP
connection, which is the part with real prerequisites: see "Two things that
are not yet solved" below before starting.
